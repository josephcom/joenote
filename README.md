...
* **Select text and JoeNote's own menu appears.** Three choices: highlight
  it, copy it, or ask Claude to sum it up in under ten words.
* **A highlight is `==text==` in the file.** Click one and you can tie a note
  to it, written in braces directly behind it — `==text=={why this matters}`.
  The two are one run of characters, so a note can never outlive the highlight
